...
namespace SongNewApi.Controllers
{
    ...
    public class SongController : ControllerBase
    {
        ...
        {
            return Ok(await _context.Songs.Where(s => !s.isRowDeleted).Skip((pageNumber-1) * pageSize).Take(pageSize).AsNoTracking().ToListAsync());
        }
        ...
        {
            var song = await _context.Songs.Where(s => !s.isRowDeleted).Include(s => s.Category).Include(s => s.Artist).AsNoTracking().FirstOrDefaultAsync(x => x.Id == id);
            if (song == null)
                return NotFound();
            else
                return Ok(song);
